...
@DgsComponent
class ProductMutationFetcher(private val store: ProductStore) {

    @DgsMutation(field = DgsConstants.MUTATION.CreateProduct)
    fun createProduct(@InputArgument input: CreateProductInput): Product {
    ...
    fun updateProduct(@InputArgument id: String, @InputArgument input: UpdateProductInput): Product? {
    ...
    fun deleteProduct(@InputArgument id: String): Boolean = store.delete(id)
}
