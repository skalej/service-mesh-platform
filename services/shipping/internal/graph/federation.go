...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Carrier":
		resolverName, err := entityResolverNameForCarrier(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Carrier": %w`, err)
		}
		switch resolverName {
		...
		case "findCarrierByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findCarrierByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindCarrierByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Carrier": %w`, err)
			}
...
func entityResolverNameForCarrier(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Carrier", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Carrier", ErrTypeNotFound))
			break
		}
		return "findCarrierByID", nil
	}
	return "", fmt.Errorf("%w for Carrier due to %v", ErrTypeNotFound,
